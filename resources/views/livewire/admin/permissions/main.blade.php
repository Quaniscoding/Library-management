...
<main class="flex-1 overflow-y-auto p-6">
    <h1 class="text-center font-bold text-2xl mb-6">Quản lý Permission</h1>
            ...
            <tbody>
                @foreach ($permissions as $permission)
                    <tr class="hover:bg-gray-100">
                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $permission->id }}</td>
                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $permission->name }}</td>
                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $permission->guard_name }}</td>
    ...
    <div x-data="{ open: @entangle('isConfirmModalOpen') }" x-show="open"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <p class="text-center mb-6">Bạn có chắc chắn muốn xóa permission này không?
            </p>
